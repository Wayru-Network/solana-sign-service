...
import { SimulateInitNfnodeParams, SimulateInitStakeParams, SimulateUnstakeParams, SimulationResult } from "@interfaces/request-transaction/simulate-request-tx.interfaces";
    ...
    async () => {
      ...
      return {
        ...
        details: {
          hasEnoughBalance:
            userBalance >
            (result1.details?.requiredBalance ?? 0) +
              (result2.details?.requiredBalance ?? 0),
          userBalance:
            result1.details?.userBalance || result2.details?.userBalance || 0,
          requiredBalance:
            (result1.details?.requiredBalance ?? 0) +
            (result2.details?.requiredBalance ?? 0),
          minimumRequired: MINIMUM_REMAINING_SOLANA_BALANCE / LAMPORTS_PER_SOL,
          breakdown: {
            transactionFee: (result1?.details?.breakdown?.transactionFee ?? 0) + (result2?.details?.breakdown?.transactionFee ?? 0),
            claimEntryRent:
        ...
        if (!hasEnoughBalance) {
          return {
            ...
            error: `User must have at least ${
              MINIMUM_REMAINING_SOLANA_BALANCE / LAMPORTS_PER_SOL
